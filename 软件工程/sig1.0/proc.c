...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  p->signal=0;                  //signals framework
  memset(p->sighandlers,0,32);  //signals framework
  return p;
}
...
fork(void)
{
  ...
  np->parent = proc;
  *np->tf = *proc->tf;
  memcpy(np->sighandlers,proc->sighandlers,32);//signal frameworks
...
exit(void)
{
 ...
 // begin_op();//signal framework
  iput(proc->cwd);
  //end_op();//signal framework
  proc->cwd = 0;

  sigsend(proc->parent->pid,2);//signal framework 2=SIGCHILDEXIT
...
//signals framework register a signal handler
void
...
void killcurproc(void) {
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    ...
    if ((p != initproc) && (p->pid != 2)) { //FIXME: state
      ...
      sigsend(p->pid, 1);//1=SIGKILLCHILD
      sigsend(p->pid, 0);//0=SIGINIT
      break;
    }
  }
